...
//Trečias greičiausias variantas 
...
void Solve(int row, int *currentCols, int *rezCols, int count, int *rez1)
{
    ...
    if(count == 0 && currentCols[SIZE/2 - 1] == EMPTY)                ///================== Sets first Bishop in the middle where dominance is biggest)
    {
        int row = SIZE/2 - 1, col = SIZE/2 - 1;
        currentCols[row] = col;
    ...
    else if(count == SIZE/2 - 1)                                      ///================== Skips the middle row and collumn because it is already set
    {
...
int IsValid(int row, int col)
{
    if(Board[row][col] != DOM_VALUE)        // Prasileidzia viena langeli (galimai del setDominance() ir removeDominance()
    {
        return 1;
    }
    return 0;
